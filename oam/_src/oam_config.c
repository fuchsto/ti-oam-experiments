...
#pragma omp target map (from: num_target_cycles)
  ...
  s_oam_config.target_freq_scale = ((t1 - t0) * 1000) + 60;

/*
  // Fine-tune, measure clock divergence for
  // calibration from first measurement:
  __ti_set_default_timeout(
      device_num, (second_ms * 1000) / s_oam_config.target_freq_scale);
